...
class Interactions(object):
    """
    Interactions object. Contains (at a minimum) pair of user-item   一对用户项
    interactions. This is designed only for implicit feedback scenarios.
    Parameters
    ----------

    file_path: file contains (user,item,rating) triplets   文件包含是(用户，项目，评级)
    user_map: dict of user mapping   用户映射字典
    item_map: dict of item mapping   项目映射字典
    """
    ...
    def tocoo(self):
        ...
        return sp.coo_matrix((data, (row, col)),
                             shape=(self.num_users, self.num_items))    #用户项目矩阵
